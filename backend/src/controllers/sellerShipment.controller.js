...
// Every lookup filters on so.seller_id = :sellerId (from sellerScope) — a seller can act on their
// own seller_orders only. Mirrors admin/shipment.controller.js, scoped down.
async function loadSellerOrder(sellerOrderId, sellerId) {
...
async function getShipment(req, res, next) {
  try {
    await loadSellerOrder(req.params.id, req.sellerId);
    const shipment = await db.queryOne('SELECT * FROM shipments WHERE seller_order_id = :id', { id: req.params.id });
...
async function createShipment(req, res, next) {
  try {
    await loadSellerOrder(req.params.id, req.sellerId);
    const result = await shiprocket.createOrder(req.params.id);
...
async function getCouriers(req, res, next) {
  try {
    const sellerOrder = await loadSellerOrder(req.params.id, req.sellerId);
    ...
    if (!address) throw ApiError.badRequest('Add your registered business address before creating shipments');
...
async function generateAWB(req, res, next) {
  try {
    await loadSellerOrder(req.params.id, req.sellerId);
    const shipment = await db.queryOne('SELECT shiprocket_shipment_id FROM shipments WHERE seller_order_id = :id', { id: req.params.id });
    ...
    await db.query(
      ...
      { id: req.params.id, note: `AWB ${result.response.data.awb_code} assigned via ${result.response.data.courier_name}` }
    );
...
async function trackShipment(req, res, next) {
  try {
    await loadSellerOrder(req.params.id, req.sellerId);
    await shiprocket.syncShipmentStatus(req.params.id);
    const shipment = await db.queryOne('SELECT * FROM shipments WHERE seller_order_id = :id', { id: req.params.id });
    const tracking = shipment
      ? await db.query('SELECT status, location, note, tracked_at FROM shipment_tracking WHERE shipment_id = :id ORDER BY tracked_at ASC', { id: shipment.id })
      : [];
...
async function cancelShipment(req, res, next) {
  try {
    await loadSellerOrder(req.params.id, req.sellerId);
    const shipment = await db.queryOne('SELECT shiprocket_order_id FROM shipments WHERE seller_order_id = :id', { id: req.params.id });

...
async function checkout(req, res, next) {
  try {
    const { address, paymentMethod, idempotencyKey, saveAddress } = req.body;
    const result = await placeOrder({ customerId: req.user.customerId, address, paymentMethod, idempotencyKey });

    // Best-effort: if the customer typed a fresh address rather than picking a saved one,
    // remember it in their address book for next time. Never blocks the order itself.
    if (!result.duplicate && saveAddress) {
      db.query(
        `INSERT INTO customer_addresses (customer_id, name, mobile, address_line1, address_line2, city, district, state, pincode, type, is_default)
         VALUES (:customerId, :name, :mobile, :address1, :address2, :city, :city, :state, :pincode, 'HOME',
           (SELECT IF(COUNT(*) = 0, 1, 0) FROM (SELECT id FROM customer_addresses WHERE customer_id = :customerId) existing))`,
        {
